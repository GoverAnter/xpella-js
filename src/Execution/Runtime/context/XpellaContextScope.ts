...
interface Scope { [name: string]: XpellaRuntimeVariable }

export class XpellaContextScope {
  private scopesStack: Array<Scope> = [{}];

  /**
   * Adds a new scope to the stack.
   * Use *initialBindings* to prefill it.
   */
  public push(initialBinding?: XpellaRuntimeVariable[]): void {
  ...
  public findInCurrentScope(identifier: string): XpellaRuntimeVariable {
    return this.scopesStack[this.scopesStack.length - 1][identifier];
  }

  /** Returns null if nothing was found. */
  public findInScopes(identifier: string): XpellaRuntimeVariable {
  ...
  */
  public createInScope(variable: XpellaRuntimeVariable, allowShadowing: boolean = true): boolean {
   ...
   */
  public setInCurrentScopeOnly(variable: XpellaRuntimeVariable): boolean {
   ...
   */
  public setInScopes(variable: XpellaRuntimeVariable): boolean {
    // Loop through stacks backward, because it is... a stack...
    for (let i = this.scopesStack.length - 1; i === 0; i--) {
